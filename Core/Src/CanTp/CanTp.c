...
CanTp_Frame CanTp_DcmFrame;//structura din DCM
CanTp_Frame CanTp_CanIfFrame;//structura din DCM
...
ComDia_StatusFlag CanTp_RxIndication(CanTp_StdId id, CanTp_MessageLength length, CanTp_Payload *data)
{
	...
	CanTp_to_PduR(&CanTp_PduRFrame);

	return COMDIA_OK;
}

void CanTp_to_PduR(CanTp_Frame *PduRFrame)
{
    PduRFrame->id = CanTp_CanTpFrame.id;
    PduRFrame->length = CanTp_CanTpFrame.length;

    for(uint8_t i = 0; i < PduRFrame->length; i++)
    {
        PduRFrame->data[i] = CanTp_CanTpFrame.data[i];
    }
...
void PduR_to_DCM(CanTp_Frame *PduRFrame)
{
    CanTp_DcmFrame.id = PduRFrame->id;
    CanTp_DcmFrame.length = PduRFrame->length;

    for(uint8_t i = 0; i < CAN_MAX_MESSAGE_LENGTH; i++)
    {
    	if (i < PduRFrame->length)
    	{
    		CanTp_DcmFrame.data[i] = PduRFrame->data[i];
    	}
    	else
    	{
...
void PduR_to_CanTp(CanTp_Frame *PduRFrame)
{
    CanTp_CanTpFrame.id = PduRFrame->id;
    CanTp_CanTpFrame.length = PduRFrame->length;

    for(uint8_t i = 0; i < PduRFrame->length; i++)
    {
        CanTp_CanTpFrame.data[i] = PduRFrame->data[i];
    }
