...
void GetResponse(CanTp_Frame *DCM_request)
{
        ...
        case DIAGNOSTIC_SESSION_CONTROL:
	{
		...
		ResponseHandling(DCM_response, &DCM_PDUR_response, DIAGNOSTIC_SESSION_CONTROL, DCM_responseData, length_output); // constructing the response - SingleFramePDU
		break;
	}
	...
	case CLEAR_DIAGNOSTIC_INFORMATION:
	{
        ...
        ResponseHandling(DCM_response, &DCM_PDUR_response, CLEAR_DIAGNOSTIC_INFORMATION, DCM_responseData, length_output);
		break;
	}
	...
	case READ_DTC_INFORMATION: //foarte multe subfunctii(21)
	{
		...
		ResponseHandling(DCM_response, &DCM_PDUR_response, READ_DTC_INFORMATION, DCM_responseData, length_output);
		break;
	}
	...
	case READ_DATA_BY_IDENTIFIER:
	{
		...
		ResponseHandling(DCM_response, &DCM_PDUR_response, READ_DATA_BY_IDENTIFIER, DCM_responseData, length_output);
		break;
	}

	case READ_MEMORY_BY_ADDRESS:
	{
		DCM_response = readMemoryByAddress(DCM_request, DCM_responseDataa, &length_output);//readDTCinformation(subfunction, DTCparametersArray-array of bytes);
		ResponseHandling(DCM_response, &DCM_PDUR_response, READ_MEMORY_BY_ADDRESS, DCM_responseData, length_output);
		break;
	}
	...
	case SECURITY_ACCESS:
	{
		...
		ResponseHandling(DCM_response, &DCM_PDUR_response, SECURITY_ACCESS, DCM_responseData, length_output);
		break;
	}
	...
	case WRITE_DATA_BY_IDENTIFIER:
	{
		...
		ResponseHandling(DCM_response, &DCM_PDUR_response, WRITE_DATA_BY_IDENTIFIER, DCM_responseData, length_output);
		break;
	}
	...
	case ROUTINE_CONTROL:
	{
		...
		ResponseHandling(DCM_response, &DCM_PDUR_response, ROUTINE_CONTROL, DCM_responseData, length_output);
		break;
	}
	...
	case WRITE_MEMORY_BY_ADDRESS:
	{
		...
		ResponseHandling(DCM_response, &DCM_PDUR_response, WRITE_MEMORY_BY_ADDRESS, DCM_responseData, length_output);
		break;
	}
    }
